...
enum nss_status
_nss_vrffiles_gethostbyname3_r (const char *name, int af, struct hostent *result,
...
again:
				...
				== NSS_STATUS_SUCCESS)
			{
				...
				/* The following piece is a bit clumsy but we want to use the
				   `LOOKUP_NAME_CASE' value.  The optimizer should do its job. */
				do {
					LOOKUP_NAME_CASE (h_name, h_aliases)
					result = old_result;
				} while ((matches = 0));
				...
				if (matches) {
					...
					/* If the real name is different add it also to the
					   ...
					   in the alias list but this is really the user's
					    problem.  */
					if (strcmp (old_result->h_name, tmp_result_buf.h_name) != 0) {
						++newaliases;
...
enum nss_status
_nss_vrffiles_gethostbyname_r (const char *name, struct hostent *result,
				char *buffer, size_t buflen, int *errnop,
				int *herrnop)
{
...
enum nss_status
_nss_vrffiles_gethostbyname2_r (const char *name, int af, struct hostent *result,
				char *buffer, size_t buflen, int *errnop,
				int *herrnop)
{
	return _nss_vrffiles_gethostbyname3_r (name, af, result, buffer, buflen,
...
enum nss_status
_nss_vrffiles_gethostbyname4_r (const char *name, struct gaih_addrtuple **pat,
				char *buffer, size_t buflen, int *errnop,
				int *herrnop, int32_t *ttlp)
{
	...
	if (status == NSS_STATUS_SUCCESS)
	{
		...
		while (1)
		{
			...
			/* Create the record the caller expects.  There is only one address.  */
		...
		/* If we have to look for multiple records and found one, this is a success.  */
